...
#define SCHED_TASK(func) FUNCTOR_BIND(&submarine, &Submarine::func, void)
...
*/
const AP_Scheduler::Task Submarine::scheduler_tasks[] PROGMEM = {
...
void Submarine::setup()
{
    ...
    // submarine does not use arming nor pre-arm checks
 ...
 */
void Submarine::mount_update(void)
{
#if MOUNT == ENABLED
    camera_mount.update();
#endif
 ...
 */
void Submarine::gcs_failsafe_check(void)
{
    if (g.fs_gcs_enabled) {
 ...
 */
void Submarine::compass_accumulate(void)
{
 ...
 */
void Submarine::update_compass(void)
{
 ...
 */
void Submarine::update_logging1(void)
{
...
/*
  log some key data - 10Hz
 */
void Submarine::update_logging2(void)
{
 ...
 */
void Submarine::update_aux(void)
{
 ...
 */
void Submarine::one_second_loop(void)
{
...
void Submarine::update_GPS_50Hz(void)
{
...
void Submarine::update_GPS_10Hz(void)
{
...
void Submarine::update_current_mode(void)
{
    switch (control_mode){
    case AUTO:
    case RTL:
...
void Submarine::update_navigation()
{
    switch (control_mode) {
    case MANUAL:
    case HOLD:
    ...
    case RTL:
        // no loitering around the wp with the submarine, goes direct to the wp position
        calc_lateral_acceleration();
        calc_nav_steer();
        if (verify_RTL()) {
            channel_throttle->servo_out = g.throttle_min.get();
