...
/*
  APMSubmarine parameter definitions
*/

#define GSCALAR(v, name, def) { submarine.g.v.vtype, name, Parameters::k_param_ ## v, &submarine.g.v, {def_value:def} }
#define GGROUP(v, name, class) { AP_PARAM_GROUP, name, Parameters::k_param_ ## v, &submarine.g.v, {group_info:class::var_info} }
#define GOBJECT(v, name, class) { AP_PARAM_GROUP, name, Parameters::k_param_ ## v, &submarine.v, {group_info:class::var_info} }
#define GOBJECTN(v, pname, name, class) { AP_PARAM_GROUP, name, Parameters::k_param_ ## pname, &submarine.v, {group_info : class::var_info} }

const AP_Param::Info Submarine::var_info[] PROGMEM = {
    ...
    // @Description: bitmask of PIDs to send MAVLink PID_TUNING messages
    // @User: Advanced
    ...
    // @Description: When enabled this tells the APM to skip the normal gyroscope calibration at startup, and instead use the saved gyro calibration from the last flight.
    // @Values: 0:Disabled,1:Enabled
    // @User: Advanced
	...
	// @Description: pin number to use to enable the throttle in auto mode. If set to -1 then don't use a trigger, otherwise this is a pin number which if held low in auto mode will enable the motor to run.
	...
	// @Description: X acceleration in meters/second/second to use to trigger the motor start in auto mode. If set to zero then auto throttle starts immediately when the mode switch happens, otherwise the submarine waits for the X acceleration to go above this value before it will start the motor
	// @Units: m/s/s
	// @Range: 0 20
	// @Increment: 0.1
	// @User: standard
    ...
    // @Description: The distance to the next turn at which the submarine reduces its target speed by the SPEED_TURN_GAIN
    // @Units: meters
    // @Range: 0 100
    // @Increment: 0.1
    // @User: Standard
    ...
    // @Description: Navigation angle threshold in degrees to switch to pivot steering when SKID_STEER_OUT is 1.
    // @Units: degrees
    // @Range: 0 360
    // @Increment: 1
    // @User: Standard
    ...
    // @Description: The maximum throttle setting to which the autopilot will apply. This can be used to prevent overheating a ESC or motor.
    // @Units: Percent
    // @Range: 0 100
    // @Increment: 1
    // @User: Standard
    ...
    // @Description: The base throttle percentage to use in auto mode. The CRUISE_SPEED parameter controls the target speed, but the submarine starts with the CRUISE_THROTTLE setting as the initial estimate for how much throttle is needed to achieve that speed. It then adjusts the throttle based on how fast the submarine is actually going.
    // @Units: Percent
    // @Range: 0 100
    // @Increment: 1
    // @User: Standard
    ...
    // @Description: Set this to 1 for skid steering controlled vehicles. When enabled, servo1 is used for the left track control, servo3 is used for right track control
    ...
    // @Description: Set this to 1 for skid steering input vehicles. When enabled, servo1 is used for the left track control, servo3 is used for right track control
    ...
    // @Description: Enable ground control station telemetry failsafe. When enabled the submarine will execute the FS_ACTION when it fails to receive MAVLink heartbeat packets for FS_TIMEOUT seconds.
    // @Values: 0:Disabled,1:Enabled
    // @User: Standard
    ...
    // @Description: The distance in meters from a waypoint when we consider the waypoint has been reached. This determines when the submarine will turn along the next waypoint path.
    // @Units: meters
    // @Range: 0 1000
    // @Increment: 0.1
    // @User: Standard
    ...
    // @Description: The maximum turning acceleration (in units of gravities) that the vehicle can handle while remaining stable.
    // @Units: gravities
    // @Range: 0.2 10
    // @Increment: 0.1
    // @User: Standard
...
void Submarine::load_parameters(void)
{
    ...
    // set a more reasonable default NAVL1_PERIOD for submarines
